...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "HUZUcoin.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "HUZUd.pid") + "\n";
#endif
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 29923, 39923) + "\n";
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core and HUZU Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fColdMasterNode = GetBoolArg("-coldmasternode", false);
    ...
    // if (fMasterNode && !fColdMasterNode) {
    //     BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
    //         strMasterNodeAddr = mne.getIp();
    //         strMasterNodePrivKey = mne.getPrivKey();
    //     }
    ...
    if (nObfuscationRounds > 16)
        nObfuscationRounds = 16;
    if (nObfuscationRounds < 1)
        nObfuscationRounds = 1;
    ...
    if (nAnonymizePivxAmount > 999999)
        nAnonymizePivxAmount = 999999;
    if (nAnonymizePivxAmount < 2)
