...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "huzu.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "huzud.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the HUZU and zHUZU money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in HUZU/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in HUZU/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, huzu, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in HUZU/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all HUZU specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-huzustake=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-huzustake=<n>", strprintf(_("Enable or disable staking functionality for HUZU inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zhuzustake=<n>", strprintf(_("Enable or disable staking functionality for zHUZU inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zHUZU minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zHUZU backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizehuzuamount=<n>     " + strprintf(_("Keep N HUZU anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The HUZU Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that HUZU is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDisableWallet) {
#endif
        if (SoftSetBoolArg("-huzustake", false))
            LogPrintf("AppInit2 : parameter interaction: wallet functionality not enabled -> setting -huzustake=0\n");
#ifdef ENABLE_WALLET
    }
#endif
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. HUZU Core is shutting down."));
...
#endif
    // Make sure only a single HUZU process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HUZU Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //HUZU: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //HUZU specific: zerocoin and spork DB's
                ...
                // HUZU: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of HUZU Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart HUZU Core to complete") << "\n";
        ...
        zwalletMain = new CzHUZUWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zHUZUWallet
        uiInterface.InitMessage(_("Syncing zHUZU wallet..."));

        bool fEnableZHuzuBackups = GetBoolArg("-backupzpiv", true);
        pwalletMain->setZHuzuAutoBackups(fEnableZHuzuBackups);
...
//    }
//
//    nAnonymizeHuzuAmount = GetArg("-anonymizehuzuamount", 0);
//    if (nAnonymizeHuzuAmount > 999999) nAnonymizeHuzuAmount = 999999;
//    if (nAnonymizeHuzuAmount < 2) nAnonymizeHuzuAmount = 2;
    ...
    LogPrintf("Anonymize HUZU Amount %d\n", nAnonymizeHuzuAmount);
    ...
    /* Denominations
       ...
       For example:
       1HUZU+1000 == (.1HUZU+100)*10
       10HUZU+10000 == (1HUZU+1000)*10
    */
