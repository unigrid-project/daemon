...
bool COutPoint::IsMasternodeReward(const CTransaction* tx) const {
	...
	return (n == tx->vout.size() - 1)
			&& (tx->vout[1].scriptPubKey != tx->vout[n].scriptPubKey);
}
...
CMutableTransaction::CMutableTransaction(const CTransaction& tx) :
		nVersion(tx.nVersion), vin(tx.vin), vout(tx.vout), nLockTime(
				tx.nLockTime), strTxComment(tx.strTxComment) {
}
...
CTransaction::CTransaction() :
		hash(0), nVersion(CTransaction::CURRENT_VERSION), vin(), vout(), nLockTime(
				0), strTxComment("") {
}
...
unsigned int CTransaction::CalculateModifiedSize(unsigned int nTxSize) const {
	...
	for (std::vector<CTxIn>::const_iterator it(vin.begin()); it != vin.end();
			++it) {
		unsigned int offset = 41U
				+ std::min(110U, (unsigned int) it->scriptSig.size());
		if (nTxSize > offset)
			nTxSize -= offset;
	}
	return nTxSize;
