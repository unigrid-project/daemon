...
static int column_alignments[] = { Qt::AlignLeft | Qt::AlignVCenter, /* status */
...
public:
	...
	void refreshWallet() {
		...
		cachedWallet.clear();
		{
			...
			for (std::map<uint256, CWalletTx>::iterator it =
					wallet->mapWallet.begin(); it != wallet->mapWallet.end();
					++it) {
	...
	void updateWallet(const uint256 &hash, int status, bool showTransaction) {
		...
		qDebug()
				<< "    inModel=" + QString::number(inModel) + " Index="
	...
	QString describe(TransactionRecord *rec, int unit) {
		{
			LOCK2(cs_main, wallet->cs_wallet);
			std::map<uint256, CWalletTx>::iterator mi = wallet->mapWallet.find(
					rec->hash);
			if (mi != wallet->mapWallet.end()) {
...
TransactionTableModel::TransactionTableModel(CWallet* wallet,
		WalletModel *parent) :
		QAbstractTableModel(parent), wallet(wallet), walletModel(parent), priv(
				new TransactionTablePriv(wallet, this)), fProcessingQueuedTransactions(
				false) {
	columns << QString() << QString() << tr("Date") << tr("Type")
			<< tr("Address")
...
QString TransactionTableModel::formatTxStatus(
	...
	case TransactionStatus::OpenUntilDate:
		status = tr("Open until %1").arg(
				GUIUtil::dateTimeStr(wtx->status.open_for));
		break;
	...
	case TransactionStatus::Confirming:
		status = tr("Confirming (%1 of %2 recommended confirmations)").arg(
				wtx->status.depth).arg(
	...
	case TransactionStatus::Immature:
		status =
				tr("Immature (%1 confirmations, will be available after %2)").arg(
						wtx->status.depth).arg(
...
QString TransactionTableModel::formatTxType(
	...
	case TransactionRecord::MNReward:
		return tr("MNReward");
	case TransactionRecord::DevFund:
		return tr("DevFund");
	default:
		return QString();
	}
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx,
		bool showUnconfirmed, BitcoinUnits::SeparatorStyle separators) const {
...
public:
	...
	void invoke(QObject *ttm) {
		...
		qDebug()
				<< "NotifyTransactionChanged : " + strHash + " status= "
						+ QString::number(status);
...
static void NotifyTransactionChanged(TransactionTableModel *ttm,
		CWallet *wallet, const uint256 &hash, ChangeType status) {
	// Find transaction in wallet
