...
bool TransactionRecord::showTransaction(const CWalletTx &wtx) {
	if (wtx.IsCoinBase()) {
		...
		if (!wtx.IsInMainChain() || wtx.GetDepthInMainChain() < 0) {
			return false;
		}
	}
	return true;
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(
		...
		if (isminetype mine = wallet->IsMine(wtx.vout[1])) {
			...
			if (wallet->IsMine(wtx.vout[nIndexMN])) {
				...
				/*&& IsMine(*wallet, destMN)*/) {
					isminetype mine = wallet->IsMine(wtx.vout[nIndexMN]);
					sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
					sub.type = TransactionRecord::DevFund;
					sub.address = CBitcoinAddress(destMN).ToString();
					sub.credit = wtx.vout[nIndexMN].nValue;
				}
			}
	...
	if (wtx.GetDepthInMainChain() >= 0) {
		...
		if (fAllFromMe && fAllToMe) {
			...
			//
			parts.append(
					TransactionRecord(hash, nTime, TransactionRecord::Other, "",
							nNet, 0));
...
// For generated transactions, determine maturity
		...
		|| (type == TransactionRecord::MNReward) || (type == TransactionRecord::DevFund)) {
