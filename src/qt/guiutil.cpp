...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a HUZU address (e.g. %1)").arg("1NS17iag9jJgTHD1VXjvLCEnZuQ3rJDE9L"));
#endif
...
bool parseBitcoinURI(const QUrl& uri, SendCoinsRecipient* out)
{
    ...
    if (!uri.isValid() || uri.scheme() != QString("HUZU"))
        return false;
...
{
    // Convert HUZU:// to HUZU:
    //
    //    Cannot handle this later, because HUZU:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith("HUZU://", Qt::CaseInsensitive)) {
        uri.replace(0, 8, "HUZU:");
    }
...
{
    QString ret = QString("HUZU:%1").arg(info.address);
    int paramCount = 0;
...
ToolTipToRichTextFilter::ToolTipToRichTextFilter(int size_threshold,
                                                 QObject* parent) : QObject(parent), size_threshold(size_threshold)
{
}
...
TableViewLastColumnResizingFixer::TableViewLastColumnResizingFixer(
    ...
    int allColsMinimumWidth) : tableView(table), lastColumnMinimumWidth(lastColMinimumWidth), allColumnsMinimumWidth(allColsMinimumWidth)
{
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HUZU.lnk";
}
...
{
    return GetAutostartDir() / "HUZU.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a HUZU.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=HUZU\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the HUZU app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add HUZU app to startup item list
...
QString formatServicesStr(quint64 mask)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (mask & check) {
            ...
            default:
                strList.append(
                    QString("%1[%2]").arg(QObject::tr("UNKNOWN")).arg(check));
            }
        }
    }
