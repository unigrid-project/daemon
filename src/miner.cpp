...
//								pblock);
				...
				txNew.vout[1].scriptPubKey = CScript()
						<< ParseHex(Params().DevPubKey().c_str())
						<< OP_CHECKSIG;
				txNew.vout[1].nValue = GetDevFundPayment(pindexPrev->nHeight,
						GetBlockValue(nHeight));
			}
	...
	do {
		...
		try {
			...
			while (true) {
				...
				if (!fProofOfStake && mapArgs["-gen"] != "1") {
					MilliSleep(20000);
					continue;
				}
...
//		LogPrintf("BitcoinMiner:: pblocktemplate.get() == 0 cointinue.....................................\n");
					...
					if (!pwallet->IsLocked()) {
						if (!pwallet->TopUpKeyPool()) {
							LogPrintf("Error in HUZUMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
							return;
						}
					}
					continue;
				...
				LogPrintf("BitcoinMiner: hashTarget: %s\n", hashTarget.ToString());
				uint256 hash;
				while (true)
				{
					// scan for solution
		...
		{
			LogPrintf("HUZUMiner terminated\n");
			throw;
		}
		...
		{
			LogPrintf("HUZUMiner runtime error: %s\n", e.what());
			return;
		}
